				...
				<form name="resultado{{$id}}" id="resultado{{$id}}" method="post" action="{{ url('Seguimiento/practica/seguimiento') }}" style="width:100%">
					...
					<table class="table table-bordered" class="table table-hover">
						<tr>
							...
							<th>
								<center>Observaciones l&iacute;der etapa productiva</center>
							</th>
						</tr>
						<tr>
							<td>
								<textArea <?php echo $disabled; ?>  name='seg_pro_obs_instructor_seguimiento' class="form-control" rows="3" maxlength="500">{{ $seg_pro_obs_instructor_seguimiento }}</textarea>
							</td>
							<td>
								<textArea <?php echo $disabled; ?> name='seg_pro_obs_lider_productiva' class="form-control" rows="3" maxlength="500">{{ $seg_pro_obs_lider_productiva }}</textArea>
							</td>
						</tr>
					</table>
				</form>
